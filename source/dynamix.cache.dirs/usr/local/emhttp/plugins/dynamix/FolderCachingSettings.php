...
<?=mk_option($ini['wait'], "", "No")?>
<?=mk_option($ini['wait'], "-w", "Yes")?>
  </select></td>
  </tr>
  <tr>
  <td>Force disks busy:</td>
  <td><select name="busy" size="1">
<?=mk_option($ini['busy'], "-B", "No")?>
<?=mk_option($ini['busy'], "", "Yes")?>
  </select></td>
  </tr>
  <tr>
  <td>Suspend during 'Mover' process:</td>
  <td><select name="suspend" size="1">
<?=mk_option($ini['suspend'], "-S", "No")?>
<?=mk_option($ini['suspend'], "", "Yes")?>
  </select></td>
  </tr>
  <tr>
  <td>Scan user shares:</td>
  <td><select name="shares" size="1">
<?=mk_option($ini['shares'], "", "No")?>
<?=mk_option($ini['shares'], "-u", "Yes")?>
  </select></td>
  </tr>
  <tr>
  <td>Minimum interval between folder scans (sec):</td>
  <td><input type="text" name="minimum" maxlength="3" value="<?=$ini['minimum']?>">default = 1 second</td>
  </tr>
  <tr>
  <td>Maximum interval between folder scans (sec):</td>
  <td><input type="text" name="maximum" maxlength="3" value="<?=$ini['maximum']?>">default = 10 seconds</td>
  </tr>
  <tr>
  <td>Maximum scan level depth:</td>
  <td><input type="text" name="depth" maxlength="4" value="<?=$ini['depth']?>">default = 9999</td>
  </tr>
  <tr>
  <td>Excluded folders (separated by comma):</td>
  <td><input type="text" name="exclude" maxlength="200" value="<?=$ini['exclude']?>">default = none</td>
  </tr>
  <tr>
  <td>Included folders (separated by comma):</td>
  <td><input type="text" name="include" maxlength="200" value="<?=$ini['include']?>">default = all</td>
  </tr>
  <tr>
  <td>User defined options:</td>
  <td><input type="text" name="other" maxlength="200" value="<?=$ini['other']?>">see <u><a href="http://lime-technology.com/forum/index.php?topic=4500.0" target="_blank">unRAID forum</a></u></td>
  </tr>
  <tr>
...
<div style="position:fixed;bottom:36px;font-style:italic;font-size:x-small"><?=exec("$fName -V")?></div>