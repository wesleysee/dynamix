...
foreach ($disks as $disk) {
  switch ($disk['name']) {
  case 'parity':
    $parity = $disk['size']*1024;
    break;
  case 'cache':
    $cache = $disk['size']*1024;
    break;
  case 'flash':
...
function total_status() {
  $.ajax({url:'<?=$stats?>', data:{type:'sum'}, success:function(string) {
    data = string.split(';');
...
<?endif;?>
  ...
  Highcharts.setOptions({
    global:{useUTC:false},
    chart:{
      style:{fontFamily:'tex-gyre,arial,sans-serif',fontSize:'10px'},
  ...
  syschart = new Highcharts.Chart({
    chart:{renderTo:'sys',events:{load:sys},type:'bar',height:<?=count($disks)*$rowHeight?>,zoomType:null},
...
<div id="totals">
 ...
 <div class="align-left"><img src="<?=$image?>/array.png" class="image-left"><strong><?=my_scale($arraysize, $units).' '.$units?></strong><br><small>Total Array Size</small></div>
 <div class="align-left"><div id="stats1" class="totalbar <?=bar_color($arraypercent)?> inside"></div><div id="stats2"><strong> <?=my_scale($arrayused, $units).' '.$units?> <img src="<?=$image?>/arrow.png" style="margin-top:-3px;"> <?=$arraypercent?>%</strong><br/><small>Total Space Used</small></div></div>
 <div class="align-left"><div class="totalbar nobar inside"></div><div id="stats3"><strong><?=my_scale($arrayfree, $units).' '.$units?> <img src="<?=$image?>/arrow.png" style="margin-top:-3px;"> <?=$freepercent?>%</strong><br/><small>Available for Data</small></div></div>
 <div class="align-right"><div class="totalbar graybar inside"></div><strong> <?=my_scale($parity, $units).' '.$units?></strong><br><small>Used for Parity</small></div>
