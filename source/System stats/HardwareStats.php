...
case 'sum':
  ...
  $display = parse_ini_file("state/plugins/dynamix.webGui.ini");
  $ini = parse_ini_file("state/plugins/dynamix.system.stats.ini");
  $data = array();
  ...
  $data[] = "<strong>".my_scale($arrayused, $units)." $units <img src='/plugins/dynamix/images/arrow.png' style='margin-top:-3px'> $arraypercent%</strong><br><small>Total Space Used</small>";
  $data[] = "<strong>".my_scale($arrayfree, $units)." $units <img src='/plugins/dynamix/images/arrow.png' style='margin-top:-3px'> $freepercent%</strong><br><small>Available for Data</small>";
  echo implode(';',$data);
  exit;
case 'sys':
...
if (empty($output)) {
  foreach ($series as $label) $output[$label][] = "";
}
foreach ($series as $label) $json[] = '"'.$label.'":['.implode(',', $output[$label]).']';
echo '{'.implode(',', $json).'}';
