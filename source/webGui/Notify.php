...
$dyn_ini = parse_ini_file("state/plugins/dynamix.webGui.ini", true);
$ini = &$dyn_ini['notify'];

$unread_path = $ini['path'].'/unread';
$archive_path = $ini['path'].'/archive';
...
case 'get':
  if (! is_dir($unread_path)) mkdir($unread_path, 0, true);
  if (isset($ini['date']) && isset($ini['time'])) {
  ...
  if (!empty($files)) {
    foreach ($files as $file) {
      ...
      $ini_array[$i]['file'] = $path_parts['filename'].'.'.$path_parts['extension'];
      $ini_array[$i] = array2json($ini_array[$i]);
      $i++;
    }
    echo array2json($ini_array);
  }
  break;
case 'archive':
