...
$confirm = &$dyn_ini['confirm'];
$display = &$dyn_ini['display'];
...
function updateTime() {
  ...
  $('#uptime').html(plus(days,'day',hour==0 && mins==0)+plus(hour,'hour',mins==0)+plus(mins,'minute',true));
  uptime += period;
  ...
  if (data) {
    ...
    $.each(json, function(i, object) {
     ...
     $.jGrowl(notification.subject+'<br>'+notification.description, {
      sticky: true,
      position: "<?=$dyn_ini['notify']['position']?>",
...
function arrayStatusbar() {
  $.ajax({url:'/plugins/webGui/include/ArrayStatusbar.php',success:function(data) {
    if (data) $('#statusbar').html(data);
<?if ($var['mdResync']!=0 && $display['refresh']):?>
    setTimeout(arrayStatusbar,<?=$display['refresh']?>);
<?endif;?>
  }});
}
setTimeout(arrayStatusbar,100);
